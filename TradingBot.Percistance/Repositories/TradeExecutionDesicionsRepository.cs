...
public class TradeExecutionDesicionsRepository(IOptions<ConnectionStrings> connections) : ITradeExecutionDesicionsRepository
{
    public async Task<long> AddDesicionAsync(TradeExecutionDecisions desicion)
    {
        const string sql = @"
                INSERT INTO trade_execution_decisions (
                    ...
                    currentcloseaboverecenthigh,
                    distancetoinvalidationpercent,
                    previouscandlebearish,
                    entrynearrecenthigh,
                    shortmaslopepercent,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        SELECT
            ...
            expectedmovepercent AS ExpectedMovePercent,
            expectedtargetprice AS ExpectedTargetPrice,
            expectedtargetsource AS ExpectedTargetSource,
            trendconfidencescore AS TrendConfidenceScore,
            marketconditionscore AS MarketConditionScore,
            volatilityregime AS VolatilityRegime,
            requiresreducedpositionsize AS RequiresReducedPositionSize,
            consecutivebullishtrendcandles AS ConsecutiveBullishTrendCandles,
            currentcloseaboverecenthigh AS CurrentCloseAboveRecentHigh,
            distancetoinvalidationpercent AS DistanceToInvalidationPercent,
            previouscandlebearish AS PreviousCandleBearish,
            entrynearrecenthigh AS EntryNearRecentHigh,
            shortmaslopepercent AS ShortMaSlopePercent,
            trendstrengthpercent AS TrendStrengthPercent,
            projectionmode AS ProjectionMode,
            projectedextension AS ProjectedExtension,
