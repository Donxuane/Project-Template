...
public sealed class NoPaidDataShortWindowSolForwardIncubationV1ReportWriter(string outputDirectory)
{
    private static readonly JsonSerializerOptions JsonOptions = new() { WriteIndented = true };

    public async Task WriteAsync(NoPaidDataShortWindowSolForwardIncubationV1RunResult result, CancellationToken ct)
    {
        Directory.CreateDirectory(outputDirectory);
        await Json("frozen-sol-candidate-summary.json", result.FrozenSummary, ct);
        await Json("sol-forward-incubation-data-coverage.json", result.DataCoverage, ct);
        await Json("sol-forward-incubation-trades.json", result.ForwardTrades.Select(NormalizedRiskPnlModule.ToReportRow).ToArray(), ct);
        await Json("sol-forward-incubation-periods.json", result.ForwardPeriods, ct);
        await Json("sol-forward-incubation-cost-sensitivity.json", result.CostSensitivity, ct);
        await Json("sol-forward-incubation-health-gates.json", result.HealthGates, ct);
        await Json("sol-forward-incubation-research-answers.json", result.Answers, ct);
        await Json("sol-forward-history.json", result.History, ct);
        await Json("sol-forward-incubation-no-trade-summary.json", result.NoTradeReasonSummary, ct);
    ...
    private async Task FrozenSummaryCsv(FrozenCandidateSummaryRow r, CancellationToken ct)
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(outputDirectory, "frozen-sol-candidate-summary.csv"), sb.ToString(), ct);
    }
    ...
    private async Task CoverageCsv(IReadOnlyList<ForwardDataCoverageRow> rows, CancellationToken ct)
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(outputDirectory, "sol-forward-incubation-data-coverage.csv"), sb.ToString(), ct);
    }
    ...
    private async Task TradesCsv(IReadOnlyList<CrossSymbolTradeRow> rows, CancellationToken ct)
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(outputDirectory, "sol-forward-incubation-trades.csv"), sb.ToString(), ct);
    }
    ...
    private async Task PeriodsCsv(IReadOnlyList<CrossSymbolPeriodRow> rows, CancellationToken ct)
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(outputDirectory, "sol-forward-incubation-periods.csv"), sb.ToString(), ct);
    }
    ...
    private async Task CostCsv(IReadOnlyList<CrossSymbolCostSensitivityRow> rows, CancellationToken ct)
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(outputDirectory, "sol-forward-incubation-cost-sensitivity.csv"), sb.ToString(), ct);
    }
    ...
    private async Task HealthGatesCsv(IReadOnlyList<ForwardHealthGateRow> rows, CancellationToken ct)
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(outputDirectory, "sol-forward-incubation-health-gates.csv"), sb.ToString(), ct);
    }
    ...
    private async Task HistoryCsv(IReadOnlyList<ForwardIncubationHistoryEntry> rows, CancellationToken ct)
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(outputDirectory, "sol-forward-history.csv"), sb.ToString(), ct);
    }
    ...
    private async Task NoTradeSummaryCsv(ForwardIncubationNoTradeReasonSummary s, CancellationToken ct)
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(outputDirectory, "sol-forward-incubation-no-trade-summary.csv"), sb.ToString(), ct);
    }
    ...
    private async Task NoTradeSummaryText(ForwardIncubationNoTradeReasonSummary s, CancellationToken ct)
    {
        ...
        await File.WriteAllTextAsync(Path.Combine(outputDirectory, "sol-forward-incubation-no-trade-summary.txt"), sb.ToString(), ct);
    }
